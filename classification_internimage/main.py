...
def parse_option():
    parser = argparse.ArgumentParser(
        'Vision-RWKV training and evaluation script', add_help=False)
    ...
    parser.add_argument(
        ...
        metavar='PATH',
        help=
        'root of output folder, the full path is <output>/<model_name>/<tag> (default: work_dirs)'
    )
...
def train_one_epoch(config,
    ...
    loss_meter = MyAverageMeter('loss', 300)
    norm_meter = MyAverageMeter('norm', 300)
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if math.isnan(loss.item()):
            # save tensors for nan analysis
            nan_skip_steps += 1
            loss = loss.detach()
            print(f'Loss is nan in iter {idx} rank {dist.get_rank()} loss {loss.item()}')
            grad_norm = None
            # saved_tensors = dict(samples=samples.cpu(),
            #                      targets=targets.cpu(),
            #                      outputs=outputs.cpu(),
            #                      model=model.module.to('cpu'))
            # torch.save(saved_tensors, 'work_dirs/saved_tensors.pth')
            sys.exit(1)
        else:
            if (idx - nan_skip_steps + 1) % accum_steps == 0:
                grad_norm = update_step(idx - nan_skip_steps, loss)
        torch.cuda.synchronize()
...
def validate(config, data_loader, model, epoch=None):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        if output.size(-1) >= 21841:  # support 47k
...
if __name__ == '__main__':
    ...
    if 'SLURM_PROCID' in os.environ and int(os.environ['SLURM_TASKS_PER_NODE'][0]) != 1:
        ...
        config.defrost()
        config.LOCAL_RANK = gpu
        print(f"set config RANK: {rank}/{world_size}")
        config.freeze()
        os.environ['LOCAL_SIZE'] = str(torch.cuda.device_count())
