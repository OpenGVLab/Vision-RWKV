...
class ImageCephDataset(data.Dataset):
    ...
    def __init__(self,
                 ...
                 target_transform=None,
                 on_memory=False):
        if '22k' in root or '22K' in root or '47k' in split:
            # Imagenet 22k & bb47k
            annotation_root = 'meta_data/'
        else:
            # Imagenet
            annotation_root = osp.join('data/imagenet', 'meta/')
...
class ParserCephImage(Parser):
    ...
    def __getitem__(self, index):
        ...
        if '47k' in self.split:
            # solve reading data on P cluster server
            filepath = filepath.replace('wwhnew:s3://INTERN1.0', 'dyc:s3://hdd_new_INTERN1_0')
        else:
